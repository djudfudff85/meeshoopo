...
def _ensure_admin():
    users = _load_users()
    user = os.getenv("ADMIN_USER", "keshvamjayam").strip()
    pwd = os.getenv("ADMIN_PASS", "jayamkeshavam")
...
async def serve_index():
    return FileResponse("public/index.html", headers={"Cache-Control": "no-store, no-cache, must-revalidate"})
